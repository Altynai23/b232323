...
public class FirstUniqueElement {
    ...
    public static void main(String[] args) {
        ...
        for (int each : nums){ // outer loop: responsible for repeating the inner loop to find frequency of each element
           ...
           int frequency = 0;
           for (int element : nums){ // outer loop: responsible for finding the frequency of one element
               if (element == each){
                   frequency++;
               }
           }
           ...
           if (frequency == 1){ // if element is unique: 2, 4
               ...
               break; //to make sure only the first unique elements is returned from the loop
           }
        }
